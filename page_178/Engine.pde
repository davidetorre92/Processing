public class Engine {
  ...
  public void neighbours() {
    int tileID = getClosestTileID(mouseX, mouseY);
    float x0 = tilemap.get(tileID).x;
    float y0 = tilemap.get(tileID).y;
    ellipse(x0, y0, rad/4., rad/4.);
    for (Tile n : getNeigh(tilemap.get(tileID), nSides, a)) {
      line(x0, y0, n.x, n.y);
    }
  }
  ...
  private void update() {
    ...
    ArrayList<Tile> updateTileList1 = updatingRule1(currentTurn);
    for (Tile updateTile : updateTileList1) {
      updateTile.updateState1();
    ...
    ArrayList<Tile> updateTileList2 = updatingRule2(currentTurn);
    for (Tile updateTile : updateTileList2) {
      updateTile.updateState2();
      for (Tile neigh : getNeigh(updateTile, nSides, a)) {
        if (!InNextTurn[neigh.tileID]) {
          nextTurn.push(neigh.tileID);
          InNextTurn[neigh.tileID] = true;
  ...
  private ArrayList<Tile> updatingRule1(IntList currentTurn) {
    ArrayList<Tile> updateTileList = new ArrayList<Tile>();
    for (int tileID : currentTurn) {
      if (tilemap.get(tileID).state==0) {
        int sum = 0;
        for (Tile neighCurrent : getNeigh(tilemap.get(tileID), nSides, a)) {
          sum += neighCurrent.state;
        }
        if (sum <=1) {
  ...
  private ArrayList<Tile> updatingRule2(IntList currentTurn) {
    ArrayList<Tile> updateTileList = new ArrayList<Tile>();
    for (int tileID : currentTurn) {
      if (tilemap.get(tileID).state==1) {
        int sum = 0;
        for (Tile neighCurrent : getNeigh(tilemap.get(tileID), nSides, a)) {
          sum += neighCurrent.state;
        }
  ...
  private void changeTileState(float x, float y) {
    ...
    if (selectedTile != -1) {
      tilemap.get(selectedTile).updateState1();
  ...
  private ArrayList<Tile> getNeigh(Tile tile, int nSides, float a) {
    ...
    ArrayList<Tile> neighsList = new ArrayList<Tile>();
    for (float angle=startAngle; angle<=TWO_PI+startAngle; angle+=dAngle/2) {
      float xNew=tile.x+2*a*cos(angle);
      float yNew=tile.y+2*a*sin(angle);
      float tiltNew = getTiltNewPolygon(tile, angle, nSides);
