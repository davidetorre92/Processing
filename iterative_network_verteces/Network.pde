class Network {
  ...
  void updatePos(){
    for(int n=0; n<nodes.length; n++){
      nodes[n].pos.x = (targetPos[n].x - nodes[n].pos.x)*0.5 + nodes[n].pos.x;
      nodes[n].pos.y = (targetPos[n].y - nodes[n].pos.y)*0.5 + nodes[n].pos.y;
    }
  }

  PVector[] initNodeskpone(int k) {
    PVector[] pos = new PVector[k+1];
    float radius = min(width, height)*0.5*radiusFrac;
    for (int s = 0; s < k+1; s++) {
      float theta = TWO_PI/(k+1) * s;
